...
const Profile = () => {
    ...
    useEffect(() => {
        axios
            .get(`http://localhost:9000/api/user/login_check`, {withCredentials: true})
            .then((res) => {
                setUser(res.data)
    ...
    const deleteFavP = (product_id, title) => {
        ...
        axios
            .post(`http://localhost:9000/api/user/delete_product/${product_id}`, fav, {withCredentials: true})
            .then((res) => {
                console.log(res.data)
            })
    }
    ...
    const deleteFavR = (recipe_id, title) => {
        const fav = {name:title};
        axios
            .post(`http://localhost:9000/api/user/delete_recipe/${recipe_id}`, fav, {withCredentials: true})
            .then((res) => {
                console.log(res.data)
            })
    }
